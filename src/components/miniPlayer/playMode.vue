<template>
    <!--TODO:制作播放模式切换组件-->
    <div class="playModeBox" ref="playModeBox">
        <!--顺序播放-->
    ...
    watch:{
        playModeSelect(newValue){
            if(newValue){
                ...
                this.$refs.playModeBox.style.display = 'flex';
                setTimeout(()=>{
                    this.$refs.playModeBox.style.opacity = 1;
                    this.$refs.playModeBox.style.borderRadius = '15px 0px 15px 0px'
                })
                

            }else{
                //隐藏
                this.$refs.playModeBox.style.opacity = 0;
                this.$refs.playModeBox.style.borderRadius = '0px 15px 0px 15px'
                this.showTime = setTimeout(()=>{
                    this.$refs.playModeBox.style.display = 'none';
                    this.showTime = 0
                },300)
            }
        }
